...
const fixturePath = new URL('artifacts/notifications-ui-fixture.json', root);
const temporaryPath = new URL('artifacts/notifications-ui-fixture.json.tmp', root);
const marker = 'karmahouse-notifications-ui-v1';
const runPrefix = 'kh-notifications-ui-';
...
const displayNames = { seller: 'Laura · Prueba de avisos', buyer: 'Daniel · Prueba de avisos', outsider: 'Cuenta ajena · Prueba aislada' };
...
async function request(path, { method = 'GET', body, token, admin = false, binary = false } = {}) {
  ...
  const response = await fetch(origin + path, {
    ...
    headers: {
      ...
      'User-Agent': 'KarmaHouse-Notifications-QA/1.0', ...(binary ? { 'x-upsert': 'false' } : {}),
    },
...
async function discoverInventoryTables() {
  ...
  requireFixture(requiredTables.every(table => tables.includes(table)), 'Apply the account, listing, negotiations and notifications migrations first.');
  return tables;
}
...
async function validateRelations(fixture, users) {
  ...
  const unrelated = (await db.query(`select
    ...
    (select count(*)::int from kh_private.notifications n
      left join public.kh_messages m on m.id=n.message_id
      left join public.kh_conversations c on c.id=n.conversation_id
      left join public.kh_negotiations p on p.id=n.negotiation_id where
      (n.recipient_id=any($1::uuid[]) or n.actor_id=any($1::uuid[])
        or n.conversation_id=any($3::uuid[]) or m.conversation_id=any($3::uuid[]) or p.conversation_id=any($3::uuid[])) and
      not(coalesce(n.recipient_id=any($1::uuid[]) and n.actor_id=any($1::uuid[])
        and n.conversation_id=any($3::uuid[]) and m.conversation_id=n.conversation_id
        and m.sender_id=n.actor_id and n.recipient_id<>n.actor_id
        and n.recipient_id in (c.buyer_id,c.seller_id) and n.actor_id in (c.buyer_id,c.seller_id)
        and (n.negotiation_id is null or p.conversation_id=n.conversation_id),false))) +
...
try {
  ...
  await db.connect();
  if (flags[0] === '--create') {
    requireFixture(!existsSync(fixturePath), 'Clean up the previous notifications fixture first.');
    stage = 'prerequisites';
    ...
    const property = await request('/rest/v1/rpc/kh_save_property', { method: 'POST', token: sellerToken, body: { p_payload: {
      ownerId: seller.id, clientRequestId: runId, title: 'Vivienda temporal · Notificaciones',
      ...
      description: 'Anuncio ficticio temporal para comprobar avisos privados entre cuentas sintéticas. Se eliminará al finalizar la prueba.',
    ...
    const notifications = (await db.query(`select count(*)::int count from kh_private.notifications
      where recipient_id=any($1::uuid[]) or actor_id=any($1::uuid[]) or conversation_id=$2`,
    [fixture.users.map(user => user.id), chat.id])).rows[0].count;
    requireFixture(notifications === 0, 'The initial fixture inboxes must be empty.');
    console.log(JSON.stringify({ result: 'notifications_fixture_ready', runId, propertyId: property.id, conversationId: chat.id,
      users: fixture.users.map(({ id, role }) => ({ id, role })),
      credentialsFile: 'artifacts/notifications-ui-fixture.json', cleanupPrerequisite: 'Sign out of the fixture UI accounts first.' }));
  } else {
    stage = 'validate_fixture';
    requireFixture(existsSync(fixturePath), 'No notifications fixture is recorded locally.');
    ...
    if (flags[0] === '--status') {
      ...
      const counts = (await db.query(`select
        ...
        (select count(*)::int from kh_private.negotiation_events e join public.kh_negotiations n on n.id=e.negotiation_id
          where n.conversation_id=any($1::uuid[])) events,
        (select count(*)::int from kh_private.notifications where conversation_id=any($1::uuid[])) notifications`,
      [scope.conversationIds])).rows[0];
      const notifications = (await db.query(`select n.id,n.seq::text,n.recipient_id,n.actor_id,n.conversation_id,
        n.message_id,n.negotiation_id,n.category,n.created_at,n.read_at,m.negotiation_event_id,
        not exists(select 1 from public.kh_user_blocks b where
          (b.blocker_id=n.recipient_id and b.blocked_id=n.actor_id)
          or (b.blocker_id=n.actor_id and b.blocked_id=n.recipient_id)) as visible
        from kh_private.notifications n join public.kh_messages m on m.id=n.message_id
        where n.recipient_id=any($1::uuid[]) order by n.seq`, [scope.ids])).rows;
      const preferences = (await db.query(`select f.user_id,coalesce(p.messages,true) messages,
        coalesce(p.visits,true) visits,coalesce(p.offers,true) offers,coalesce(p.version,0) version
        from unnest($1::uuid[]) f(user_id) left join kh_private.notification_preferences p on p.user_id=f.user_id
        order by f.user_id`, [scope.ids])).rows;
      const inboxes = users.map(user => {
        const own = notifications.filter(notification => notification.recipient_id === user.id);
        const visible = own.filter(notification => notification.visible);
        return { userId: user.id, role: user.role, total: own.length, visible: visible.length,
          unread: visible.filter(notification => notification.read_at === null).length,
          readThrough: visible.reduce((latest, notification) => BigInt(notification.seq) > BigInt(latest) ? notification.seq : latest, '0'),
          categories: ['message', 'visit', 'offer'].map(category => ({ category,
            total: own.filter(notification => notification.category === category).length,
            visibleUnread: visible.filter(notification => notification.category === category && notification.read_at === null).length })),
        };
      });
      await db.query('rollback');
      console.log(JSON.stringify({ result: 'notifications_fixture_status', runId: fixture.runId,
        users: users.map(({ id, role, exists }) => ({ id, role, exists })), properties, conversations, negotiations, counts,
        notifications, preferences, inboxes,
      ...
      const remaining = (await db.query(`select
        ...
        (select count(*)::int from kh_private.notifications where recipient_id=any($1::uuid[]) or actor_id=any($1::uuid[]) or conversation_id=any($4::uuid[])) notifications,
        (select count(*)::int from kh_private.notification_preferences where user_id=any($1::uuid[])) notification_preferences,
      ...
      console.log(JSON.stringify({ result: 'notifications_fixture_cleanup_verified', remaining, inventoryUnchanged: true }));
    }
  }
} catch (error) {
  await db.query('rollback').catch(() => {});
  console.error(JSON.stringify({ result: 'notifications_fixture_failed', stage,
