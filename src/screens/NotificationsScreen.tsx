  ...
  return <>
    ...
    <FlatList data={list.items} keyExtractor={item => item.id} contentContainerStyle={styles.list}
      ...
      ListEmptyComponent={!list.ready ? <ActivityIndicator color={colors.primary} style={styles.loading} />
        ...
        : <EmptyState icon={list.unreadOnly ? 'checkmark-done-outline' : 'notifications-outline'} title={list.unreadOnly ? 'Estás al día' : 'Aquí llegan tus novedades'} description={list.unreadOnly ? 'No tienes avisos sin leer. Puedes consultar los anteriores en Todas.' : 'Cuando recibas mensajes o novedades sobre visitas y ofertas, las encontrarás aquí.'} action={<Button label={list.unreadOnly ? 'Ver todos los avisos' : 'Ir a mensajes'} secondary onPress={() => list.unreadOnly ? store.setUnreadOnly(false) : router.push('/messages')} />} />}
